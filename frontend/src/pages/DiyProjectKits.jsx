...
export default function DiyProjectKits() {
  // --- States ---
    ...
    <div className="bg-gray-50 min-h-screen pt-4 pb-12">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <nav className="flex items-center space-x-2 text-xs text-gray-500 mb-6 bg-white py-3 px-4 rounded-md shadow-sm border border-gray-100">
          ...
          <span className="text-gray-800 font-medium">DIY Project Kits</span>
        </nav>

        {/* Catalog Banner */}
        <div className="bg-[#fb7b02] text-white rounded-lg p-6 md:p-8 mb-8 shadow-sm relative overflow-hidden">
          ...
          <div className="relative z-10 max-w-xl">
            <span className="bg-[#003e8b] text-[10px] uppercase font-bold px-2 py-0.5 rounded shadow-sm text-white">
              Do-It-Yourself
            </span>
            ...
            <p className="text-gray-100 text-xs md:text-sm leading-relaxed">
              Explore your creativity and build fantastic electronics and robotics projects from scratch! Designed with simple steps, ideal for students, hobbyists, and beginners.
            </p>
          </div>
        </div>
            ...
            <div className="bg-white p-5 rounded-lg shadow-sm border border-gray-100 sticky top-24 max-h-[85vh] overflow-y-auto space-y-5">
              <div className="flex items-center justify-between border-b border-gray-100 pb-3 mb-2">
                <span className="font-bold text-sm text-gray-800 flex items-center gap-1.5">
                  <Filter className="h-4 w-4 text-[#fb7b02]" />
                  Filters
                </span>
                <button
                  ...
                  {subcategories.map((sub) => (
                    <label key={sub.id} className="flex items-center gap-2.5 text-xs text-gray-600 hover:text-gray-800 cursor-pointer">
                      <input
                        ...
                        className="rounded border-gray-300 text-[#fb7b02] focus:ring-[#fb7b02] h-3.5 w-3.5 cursor-pointer"
                      />
                      <span>{sub.name}</span>
                    </label>
                  ))}
              ...
              <div className="border-t border-gray-100 pt-4">
                ...
                <div className="grid grid-cols-2 gap-2 mb-3">
                  <input
                    ...
                    className="w-full text-xs px-2.5 py-1.5 border border-gray-200 rounded focus:outline-none focus:border-[#fb7b02] bg-gray-50 focus:bg-white"
                  />
                  <input
                    type="number"
                    placeholder="Max"
                ...
                <button
                  ...
                  className="w-full bg-[#fb7b02] hover:bg-[#e06c00] text-white text-xs font-bold py-1.5 rounded shadow-sm transition border-0 cursor-pointer"
                >
                  Apply Price
                </button>
              </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <select
                    ...
                    className="text-xs px-2.5 py-1.5 border border-gray-200 rounded focus:outline-none focus:border-[#fb7b02] bg-gray-50 font-medium cursor-pointer"
                  >
            ...
            {loading ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                {filteredProjects.map((project) => (
                  <KitCard key={project.id} product={project} categorySlug="diy-project-kits" />
                ))}
              </div>
            ) : (
              <div className="bg-white rounded-lg p-12 text-center border border-gray-100 shadow-sm flex flex-col items-center">
                ...
                <p className="text-gray-400 text-xs max-w-sm mb-4 leading-relaxed">
                  We couldn't find any DIY kits matching your current filters. Try relaxing your filters or click below to clear everything.
                </p>
                <button
                  onClick={handleClearFilters}
                  className="bg-[#fb7b02] hover:bg-[#e06c00] text-white text-xs font-bold px-4 py-2 rounded shadow-sm transition border-0 cursor-pointer"
                >
                  Clear All Filters
                </button>
              </div>
        ...
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <div className="fixed inset-y-0 left-0 w-80 max-w-full bg-white shadow-xl flex flex-col p-5 animate-slide-in">
                  ...
                  {subcategories.map((sub) => (
                    <label key={sub.id} className="flex items-center gap-2.5 text-xs text-gray-600 cursor-pointer">
                      <input
                        ...
                        checked={selectedSubcategories.includes(sub.id)}
                        onChange={() => handleSubcategoryToggle(sub.id)}
                        className="rounded border-gray-300 text-[#fb7b02] h-3.5 w-3.5"
                      />
                      <span>{sub.name}</span>
                    </label>
                  ))}
            ...
            <div className="border-t border-gray-100 pt-4 flex gap-2.5 mt-auto bg-white">
              ...
              <button
                onClick={handleApplyFilters}
                className="flex-1 bg-[#fb7b02] hover:bg-[#e06c00] text-white text-xs font-bold py-2 rounded shadow-sm border-0 cursor-pointer"
              >
                Apply
              </button>
            </div>
